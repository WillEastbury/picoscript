...
(function (root, factory) {
  ...
  function sE(s,d){var p="    ".repeat(d||0);if(!s)return"";if(s.t==="Decl"||s.t==="Dim"||s.t==="Let"||s.t==="Assign")return p+"Set "+s.name+" to "+exprStr(s.value||s.init||{t:"Num",value:0},"english")+".";if(s.t==="IncDec")return p+(s.delta>0?"Increase ":"Decrease ")+s.name+" by 1.";if(s.t==="If"){var arms=s.arms||[[s.cond,s.then]];var els=s.els;var o=p+"If "+exprStr(arms[0][0],"english")+":\n";arms[0][1].forEach(function(st){o+=sE(st,d+1)+"\n";});if(els){o+=p+"Otherwise:\n";(Array.isArray(els)?els:[els]).forEach(function(st){o+=sE(st,d+1)+"\n";});}return o.trimEnd();}if(s.t==="While"){var o=p+"While "+exprStr(s.cond,"english")+":\n";s.body.forEach(function(st){o+=sE(st,d+1)+"\n";});return o.trimEnd();}if(s.t==="ForTo"||s.t==="ForEach"){var n=s.count||s.end;return p+"For each "+(s.v||"i")+" from 0 to "+exprStr(n,"english")+":\n"+(s.body||[]).map(function(st){return sE(st,d+1);}).join("\n");}if(s.t==="Return")return p+"Return"+(s.value?" "+exprStr(s.value,"english"):"")+".";if(s.t==="Break")return p+"Stop.";if(s.t==="Skip"||s.t==="Continue")return p+"Skip.";if(s.t==="Goto")return p+"Go to "+s.label+".";if(s.t==="Label")return p+"Label "+s.name+".";if(s.t==="Gosub"){var a=(s.args||[]).map(function(x){return exprStr(x,"english");});return p+"Call "+s.name+(a.length?"("+a.join(", ")+")":"")+".";} if(s.t==="CallStmt")return p+exprStr(s.call,"english")+".";if(s.t==="ExprStmt"){if(s.expr&&s.expr.t==="Call"&&!s.expr.ns&&s.expr.method==="print")return p+"Print "+exprStr(s.expr.args[0],"english")+".";return s.expr?p+exprStr(s.expr,"english")+".":"";}if(s.t==="Print")return p+"Print "+exprStr(s.value,"english")+".";if(s.t==="Switch"||s.t==="Dispatch"){var o=p+"Choose "+exprStr(s.expr,"english")+":\n";(s.cases||[]).forEach(function(c){o+=p+"    When "+exprStr(c[0],"english")+":\n";c[1].forEach(function(st){o+=sE(st,d+2)+"\n";});});if(s.def||s.els){o+=p+"    Otherwise:\n";(s.def||s.els||[]).forEach(function(st){o+=sE(st,d+2)+"\n";});}return o.trimEnd();}if(s.t==="DoLoop"||s.t==="DoWhile"){var cond=s.botCond||s.cond||{t:"Num",value:1};return p+"Repeat:\n"+(s.body||[]).map(function(st){return sE(st,d+1);}).join("\n")+"\n"+p+"Until "+exprStr(cond,"english")+".";}if(s.t==="ServerMain")return (s.body||[]).map(function(st){return sE(st,d);}).join("\n");return p+"' "+s.t;}
  ...
  function sCob(s,d){var p="    ".repeat(d||0);if(!s)return"";if(s.t==="Decl"||s.t==="Dim"||s.t==="Let"||s.t==="Assign")return p+"COMPUTE "+s.name+" = "+exprStr(s.value||s.init||{t:"Num",value:0},"cobol")+".";if(s.t==="Print")return p+"DISPLAY "+exprStr(s.value,"cobol")+".";if(s.t==="ExprStmt"){if(s.expr&&s.expr.t==="Call"&&!s.expr.ns&&s.expr.method==="print")return p+"DISPLAY "+exprStr(s.expr.args[0],"cobol")+".";return s.expr?p+exprStr(s.expr,"cobol")+".":"";}if(s.t==="Gosub")return p+"PERFORM "+s.name+".";if(s.t==="CallStmt")return p+exprStr(s.call,"cobol")+".";if(s.t==="Return")return p+"STOP RUN.";if(s.t==="IncDec")return p+"COMPUTE "+s.name+" = "+s.name+" + "+s.delta+".";if(s.t==="If"){var arms=s.arms||[[s.cond,s.then]],els=s.els,o=p+"IF "+exprStr(arms[0][0],"cobol");arms[0][1].forEach(function(st){o+="\n"+sCob(st,d+1);});for(var i=1;i<arms.length;i++){o+="\n"+p+"ELSE\n"+p+"    IF "+exprStr(arms[i][0],"cobol");arms[i][1].forEach(function(st){o+="\n"+sCob(st,d+2);});o+="\n"+p+"    END-IF.";}if(els){o+="\n"+p+"ELSE";(Array.isArray(els)?els:[els]).forEach(function(st){o+="\n"+sCob(st,d+1);});}return o+"\n"+p+"END-IF.";}if(s.t==="While")return p+"PERFORM UNTIL NOT ("+exprStr(s.cond,"cobol")+")\n"+(s.body||[]).map(function(st){return sCob(st,d+1);}).join("\n")+"\n"+p+"END-PERFORM.";if(s.t==="ForTo"){var step=s.step?exprStr(s.step,"cobol"):"1";return p+"PERFORM VARYING "+s.v+" FROM "+exprStr(s.start,"cobol")+" BY "+step+" UNTIL "+s.v+" > "+exprStr(s.end,"cobol")+"\n"+(s.body||[]).map(function(st){return sCob(st,d+1);}).join("\n")+"\n"+p+"END-PERFORM.";}if(s.t==="ForEach")return p+"PERFORM VARYING "+s.v+" FROM 0 BY 1 UNTIL "+s.v+" >= "+exprStr(s.count,"cobol")+"\n"+(s.body||[]).map(function(st){return sCob(st,d+1);}).join("\n")+"\n"+p+"END-PERFORM.";if(s.t==="Break")return p+"EXIT PERFORM.";if(s.t==="Skip"||s.t==="Continue")return p+"EXIT PERFORM CYCLE.";if(s.t==="Goto")return p+"GO TO "+s.label+".";if(s.t==="Label")return p+s.name+".";if(s.t==="Switch"){var arms2=[];(s.cases||[]).forEach(function(c){arms2.push([{t:"Cmp",cond:"EQ",lhs:s.expr,rhs:c[0]},c[1]]);});return sCob({t:"If",arms:arms2,els:s.def},d);}if(s.t==="ServerMain")return (s.body||[]).map(function(st){return sCob(st,d);}).join("\n");return p+"*> "+s.t;}
