...
- **Tensor / transformer primitives:** `Tensor.*` and `BitLinear.*` are span-based inference kernels. They cover int8 dot/matvec, BitNet ternary matvec, int32 elementwise ops, RMSNorm, RoPE, softmax-style Q15 weights and argmax. Outputs are int32 big-endian spans except scalar methods. Hosts may accelerate them without changing source.
