...
- **Picowal PR78 host facades:** `Storage.Ready/IsUserPack` expose disk-ready/user-pack policy; `Query.BuildLookupFilter/BuildManyToManyMap` expose bounded relation query builders; `Search.*` exposes host search primitives (index, lexical/vector/hybrid query, score/result/plan) with a deterministic scalar reference implementation and host-accelerated bindings expected in production.
- **Tensor / transformer primitives:** `Tensor.*`, `BitLinear.*`, `Tokenizer.*`, `Model.*`, `Kv.*`, and `Sampling.*` are span-based inference building blocks. They cover int8 dot/matvec, BitNet ternary formats (2-bit, bitmap, base3), int32 elementwise ops, RMSNorm, RoPE, softmax-style Q15 weights, argmax/top-k, byte-fallback tokenization, model/tensor metadata and KV cache records. Outputs are int32 big-endian spans except scalar methods. Hosts may accelerate them without changing source.
- **OS-worker process lifecycle:** `Process.Self/Parent/Spawn/Exit/Kill/Status/Wait/Args` provide a deterministic simulated process table (single-process; Spawn logs + returns fake pid; Status/Wait return immediately). PIOS will bind real spawn/terminate/fork. `Env.Get/Set/Count/Key` provide process-scoped key-value environment variables. All CAP_PROCESS-gated (1<<16).
- **Timers and scheduler:** `Timer.After(ms)/Every(ms)/Cancel/Elapsed` create one-shot and repeating timers. `Scheduler.Tick(ms)` advances simulated time and fires expired timers, injecting `EVENT_TIMER` (type=100, target=handle) events into the `Event.*` queue. Deterministic manual time for tests; PIOS binds real interrupts. CAP_TIMER-gated (1<<17).
- **Principal / capability / sandbox:** `Principal.Current/HasRole/Claims` query the current identity. `Capability.Has/Request/Drop` dynamically manage granted capability bits. `Sandbox.Deny` irrevocably revokes a capability. Injectable principals/roles/claims in the reference VM; PIOS fills real authz. CAP_PRINCIPAL-gated (1<<18).
- **Error handling (Phase 1 — global handler):** `Error.SetHandler(pc)` installs a global fault handler. When a `PicoFault` occurs during execution, the VM jumps to the handler PC instead of halting, with `Error.Code/Detail` available for inspection and `Error.Resume/Clear` for recovery. No capability needed (pure language primitive).
- **Capsule execution (inter-card module switching):** `Capsule.Call/Schedule/Jump/LoadModule/RunModule` provide inter-card bytecode execution. Call runs a card's bytecode synchronously (simulated: returns 0). Schedule binds a card to future event dispatch. Jump transfers execution permanently (halts current program). LoadModule/RunModule separate loading from execution. CAP_CAPSULE_EXEC-gated (1<<19).
...
- `Decimal.Parse/ToString/Add/Sub/Mul/Div/Compare/ToInt(...)` for fractional-value arithmetic. Number is 32-bit-integer only (Number.Parse truncates any fraction); Decimal preserves it as a Q16.16 fixed-point value in a plain 32-bit register (same encoding as Maths.Sin/Cos/Exp/Log below), byte-identical across the Python/JS VMs. Decimal.ToString renders the shortest decimal string that round-trips to the exact same value (matches how e.g. Python/JS render IEEE754 floats), so common values like "19.99" or "5.10" display cleanly rather than showing binary-fraction noise. No capability needed (pure language primitive).
