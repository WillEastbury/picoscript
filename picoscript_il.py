...
def lower_to_js(insts: List[Inst], module_name: str = "pico", opt: bool = True) -> str:
    ...
    out.append("  function makeRuntime(options) {")
    ...
    out.append("    if (_PV) {")
    out.append("      // First-class host: delegate to the shared JS VM host (picovm.js) by hook")
    out.append("      // code, so the span/string namespaces run at full parity with no VM loop.")
    out.append("      var hv = new _PV(options || {});")
    out.append("      var _imm = function (c) { return c <= 0xff ? (0x7000 | c) : (0x6000 | (c & 0xfff)); };")
    out.append("      var _call = function (c, a, b) { hv.regs[1] = a | 0; hv.regs[2] = b | 0; hv.regs[0] = 0; hv._host(c, 0, 1, 2, _imm(c)); return hv.regs[0] | 0; };")
    out.append("      return {")
    out.append("        cards: {}, output: hv.output, httpStatus: -1, httpType: null, closed: false,")
    ...
    out.append("        mem: hv.mem, dotLen: 0,")
    out.append("        memGet: function (a) { return _call(0x37, a, 0); },")
    out.append("        memSet: function (a, v) { _call(0x36, a, v); },")
    out.append("        ioWrite: function (b) { _call(0x72, b, 0); },")
    out.append("        dotLenSet: function (n) { _call(0x56, n, 0); },")
