...
{
    if (contains(name, "_blocks") || contains(name, "_scales")) return -1;
...
int main(int argc, char **argv)
{
    ...
    weight_map map = {0};
    FILE *manifest, *out;
    int emitted = 0, rejected_mxfp4 = 0;
    const char *options;
    if (argc < 5) {
        fprintf(stderr,
    ...
    fputs("int quantizeTensor(int modelPath, int weightName, int calibrationPath, "
          "int calibrationName, int outputPath) {\n", out);
    fputs("    int model = Shard.Load(modelPath, \"mmap\");\n"
          ...
          "    int weightOptions = String.Concat(\"tensor=\", weightName);\n"
    ...
    while (fgets(line, sizeof(line), manifest)) {
        char *fields[4], model_path[4096];
        const char *shard;
        int allowed;
        size_t n = strlen(line);
        while (n && (line[n - 1] == '\n' || line[n - 1] == '\r')) line[--n] = '\0';
        if (!n || line[0] == '#') continue;
        if (!split_manifest(line, fields, 4)) {
        ...
        if (allowed < 0) {
            fprintf(stderr, "MXFP4 tensor requires dequantization before CAT-Q: %s\n", fields[0]);
            rejected_mxfp4++;
            continue;
        }
        if (!allowed) {
        ...
        pico_string(out, fields[0]); fputs(", ", out);
