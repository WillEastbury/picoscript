 ...
 *     [weight_view]  [job_options]  [target_tensor]
 *
 * Output:
...
int main(int argc, char **argv)
{
    ...
    fputs(";\n"
          "    if (Span.Len(jobOptions) > 0) {\n"
          "        catqOptions = String.Concat(catqOptions, \";\");\n"
          "        catqOptions = String.Concat(catqOptions, jobOptions);\n"
          "    }\n"
          "    int target = 0;\n"
          "    int context = 0;\n"
          "    if (Span.Len(targetName) > 0) {\n"
          "        int targetOptions = String.Concat(\"tensor=\", targetName);\n"
          "        target = Tensor.Map(calShard, targetOptions);\n"
          "        if (target == 0) { raise 3007; }\n"
          "        context = CatQ.CalibrateTarget(calibration, target, catqOptions);\n"
          "    } else {\n"
          "        context = CatQ.Calibrate(calibration, catqOptions);\n"
          "    }\n"
          ...
          "    if (target != 0) { Tensor.Release(target); }\n"
    ...
    while (fgets(line, sizeof(line), manifest)) {
        char *fields[7], model_path[4096], weight_options[1024], scale_name[512];
        ...
        field_count = split_manifest(line, fields, 7);
        if (field_count < 4) {
        ...
        pico_string(out, fields[3]); fputs(", ", out);
        pico_string(out, field_count >= 6 ? fields[5] : ""); fputs(", ", out);
        pico_string(out, field_count >= 7 ? fields[6] : ""); fputs(");\n", out);
