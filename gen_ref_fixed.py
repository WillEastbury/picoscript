...
.method-sig { background: white; padding: 10px; border-radius: 4px; font-family: monospace; margin: 10px 0; font-size: 12px; }
...
<div class="tree-title" onclick="toggleLanguageSyntax()">
...
for keyword in LANGUAGE_SYNTAX:
    html_content += f'<div class="tree-item" onclick="selectKeyword(this, \'{keyword}\')">{keyword}</div>\n'
...
<div class="tree-section">
<div class="tree-title" onclick="toggleNamespacesSection()">
<span class="tree-toggle">v</span>
...
function toggleLanguageSyntax() {
    const items = document.getElementById('lang-items');
    const toggle = event.target.closest('.tree-title').querySelector('.tree-toggle');
...
function toggleNamespacesSection() {
...
function selectKeyword(el, keyword) {
    clearSelection();
...
function selectMethod(e, ns, method) {
    ...
    const nsTab = document.querySelector('button[data-tab="namespaces"]');
    if (nsTab) {
        nsTab.click();
    }
...
print(f'Generated docs/PICOSCRIPT_REFERENCE.html')
