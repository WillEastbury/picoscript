...
<style>
  ...
  * { box-sizing:border-box; margin:0; padding:0; }
  ...
  .file-badge.k-rbac { background:#66d9ef; }
...
<div class="main">
      ...
      <div class="file-sidebar" id="fileSidebar">
        ...
        <div class="file-body">
          <div class="file-actions">
            ...
            <button class="ghost" onclick="rbacQuickNew()" title="Design roles, pack permissions, and row-level security predicates">+ RBAC/RLS</button>
          </div>
      ...
      <div class="ide-editor">
        ...
        <div id="schemaDesigner" class="wf-designer" style="display:none">
          <div class="wf-add">
            ...
            <button class="ghost" onclick="schemaPushLive()" title="PUT this schema to the configured live server">Push to live server</button>
        ...
        <div id="rbacDesigner" class="wf-designer" style="display:none">
          <div class="wf-add">
            <button class="ghost" onclick="rbacOpenJson()" title="View / edit the backing RBAC/RLS JSON">&#123; &#125; JSON</button>
            <span class="muted" style="font-size:11px">design roles, per-pack action permissions, and row-level security predicates (same query syntax as the Query panel); Cards/Query enforce these for the "current role" selected below</span>
          </div>
          <div id="rbacBody"></div>
        </div>
        ...
        <div class="wf-modal" id="rbacJsonModal">
          <div class="wf-modal-box">
            <div class="wf-modal-head">RBAC/RLS JSON (roles + permissions + row policies)<button class="ghost" onclick="rbacCloseJson()">&#10005;</button></div>
            <div class="wf-modal-body"><textarea id="rbacJsonText" spellcheck="false"></textarea><div id="rbacJsonErr" class="cerr"></div></div>
            <div class="wf-modal-foot"><button class="act" onclick="rbacApplyJson()">Apply</button><button class="ghost" onclick="rbacCloseJson()">Cancel</button></div>
          </div>
        </div>
      </div>
    </div>
  ...
  <div class="flyout-trigger" id="flyoutTriggers">
    ...
    <button onclick="openToolPanel('routes')">Routes</button>
  </div>
  ...
  <div class="tool-panel" id="toolPanel">
    <div class="tool-head">
      <div class="tool-tabs">
        ...
        <button data-tool="routes" onclick="selectToolTab('routes')">Routes</button>
      </div>
    ...
    <div class="tool-body">
      ...
      <div class="tool-tab" id="tool-cards">
        ...
        <div id="liveServerBar" style="display:flex;gap:6px;align-items:center;margin-bottom:6px;font-size:11px">
          <span id="liveServerStatus" class="muted">offline (localStorage simulator)</span>
          <button class="ghost" style="padding:1px 7px" onclick="liveServerPrompt()">Set live server&hellip;</button>
        </div>
        <div id="rbacRoleBar" style="display:flex;gap:6px;align-items:center;margin-bottom:6px;font-size:11px">
          <span class="muted">current role:</span>
          <input id="rbacRoleInput" placeholder="(unrestricted)" style="width:100px" oninput="rbacSetCurrentRole(this.value.trim());cardRender();queryRenderChips();" title="Simulates access as this role for the RBAC/RLS designer's permissions/policies. Blank = unrestricted.">
        </div>
      ...
      <div class="tool-tab" id="tool-query">
        <h3>Query</h3>
        <div id="liveServerStatus2" class="muted" style="font-size:11px;margin-bottom:6px">offline (localStorage simulator)</div>
      ...
      <div class="tool-tab" id="tool-routes">
        <h3>Program Exits (routes)</h3>
        <p style="color:var(--muted);font-size:12px;line-height:1.5">Bind a method+path to a runnable <b>program exit</b>: a workflow/code file (compiled &amp; run through the HTTP Simulator's Send action, same as today), a live <b>Query</b> against a pack, or a pack <b>List</b>. Routes are also what a deployed card Package carries (see Package export).</p>
        <table class="wal" style="margin-bottom:8px"><thead><tr><th>method</th><th>path</th><th>type</th><th>target</th><th></th></tr></thead><tbody id="routesBody"></tbody></table>
        <div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap;margin-bottom:8px">
          <select id="routeMethod"><option>*</option><option>GET</option><option>POST</option><option>PUT</option><option>DELETE</option></select>
          <input id="routePath" placeholder="/orders" style="width:110px">
          <select id="routeType" onchange="routesRenderTargetInputs()"><option value="code">code/workflow file</option><option value="query">Query (pack)</option><option value="list">List (pack)</option></select>
          <span id="routeTargetInputs"></span>
          <button class="ghost" onclick="routeAdd()">+ route</button>
        </div>
        <div id="routesMsg" class="cerr"></div>
      </div>
    </div>
  </div>
</div>
...
function enterStaticMode(name,kind){
  ...
  if(typeof rbacToggle==='function') rbacToggle(kind==='rbac', name);
}
function exitStaticMode(){
  ...
  if(typeof rbacToggle==='function') rbacToggle(false);
  if(typeof wfToggle==='function') wfToggle();
  if(typeof reportToggle==='function') reportToggle();
}
...
function fileKind(name){
  var n=String(name||'').toLowerCase();
  if(/\.rbac\.json$/.test(n)) return 'rbac';
...
function fileIcon(kind){ return {folder:'\uD83D\uDCC1',card:'\uD83D\uDDC3',schema:'\uD83E\uDDE9',event:'\u26A1',ontology:'\uD83D\uDD78\uFE0F',rbac:'\uD83D\uDD10',source:'\uD83D\uDD16',static:'\uD83D\uDCC4',code:'\u2039\u203A'}[kind]||'\uD83D\uDCC4'; }
...
function fileDefaultSrc(kind){
  ...
  if(kind==='rbac') return JSON.stringify({roles:[],permissions:[],rowPolicies:[]},null,2);
  return '';
}
...
// A route's `type` distinguishes what runs it as a "program exit":
//  'code'  (default, backward compatible with pre-existing {method,path,file}
//           routes carried by deploy packages) -- runs a code/workflow file
//           through the existing compile+VM path, same as sendRequest always did.
//  'query' -- runs a live Query against `pack` (offline STORE or, in principle,
//           the same dual-mode as Cards/Query -- see routeRunQuery) and returns
//           JSON rows directly, no VM/compile involved.
//  'list'  -- returns the full pack listing as JSON, mirroring picoweb's
//           GET /wal/list/{pack} convention.
function matchRoute(methodCode,path){
  ...
  for(var i=0;i<rs.length;i++){var r=rs[i];
    ...
    if(!mOk||!pOk) continue;
    var type=r.type||'code';
    if(type==='code'&&r.file&&filesRead()[r.file]) return r;
    if((type==='query'||type==='list')&&r.pack) return r;
  }
  return null;
}
function methodCodeName(m){ if(typeof m==='number')return m; return (typeof methodCode==='function')?methodCode(String(m)):0; }

// ---- Program Exits designer (visual UI over the routes data layer above) ----
// routesRead/routesWrite/matchRoute already existed (consumed by sendRequest
// and by exportPackage/import for deploy packages) but had no visual editor --
// a route could only be created by hand-authoring a package JSON. This adds
// the missing UI, plus the 'query'/'list' route types (see sendRequestDataRoute)
// so a Query or a pack listing can be bound to a method+path directly, not
// just a code/workflow file.
function routableFiles(){
  var files=filesRead();
  return Object.keys(files).filter(function(n){return fileKind(n)==='code';}).sort();
}
function routesRenderTargetInputs(){
  var typeEl=document.getElementById('routeType'), host=document.getElementById('routeTargetInputs');
  if(!typeEl||!host) return;
  var type=typeEl.value;
  if(type==='code'){
    var files=routableFiles();
    host.innerHTML='<select id="routeFile">'+(files.length?files.map(function(f){return '<option>'+esc(f)+'</option>';}).join(''):'<option value="">(no code files)</option>')+'</select>';
  } else if(type==='list'){
    host.innerHTML='<input id="routePack" placeholder="pack name" style="width:100px">';
  } else {
    host.innerHTML='<input id="routePack" placeholder="pack name" style="width:100px"> <input id="routeQuery" placeholder="qty &gt; 40 (optional)" style="width:150px">';
  }
}
function routesMsg(m,err){ var e=document.getElementById('routesMsg'); if(!e) return; e.textContent=m||''; e.style.color=err?'#ff7b72':'var(--muted)'; }
function routeAdd(){
  var method=document.getElementById('routeMethod').value, path=(document.getElementById('routePath').value||'').trim(), type=document.getElementById('routeType').value;
  if(!path){ routesMsg('path is required',true); return; }
  var route={method:method,path:path,type:type};
  if(type==='code'){
    var fEl=document.getElementById('routeFile');
    if(!fEl||!fEl.value){ routesMsg('no code file selected (create one first)',true); return; }
    route.file=fEl.value;
  } else {
    var pEl=document.getElementById('routePack'), pack=(pEl?pEl.value:'').trim();
    if(!pack){ routesMsg('pack name is required',true); return; }
    route.pack=pack;
    if(type==='query'){ var qEl=document.getElementById('routeQuery'); route.query=(qEl?qEl.value:'').trim(); }
  }
  var rs=routesRead(); rs.push(route); routesWrite(rs);
  routesMsg((route.method||'*')+' '+route.path+' added');
  routesRender();
}
function routeRemove(idx){ var rs=routesRead(); rs.splice(idx,1); routesWrite(rs); routesRender(); }
function routesRender(){
  var host=document.getElementById('routesBody'); if(!host) return;
  var rs=routesRead();
  if(!rs.length) host.innerHTML='<tr><td colspan="5" style="color:var(--muted)">no routes yet -- program exits are opt-in</td></tr>';
  else host.innerHTML=rs.map(function(r,i){
    var type=r.type||'code';
    var target=(type==='code')?esc(r.file||''):('pack: '+esc(r.pack||'')+((type==='query'&&r.query)?(', query: '+esc(r.query)):''));
    return '<tr><td>'+esc(r.method||'*')+'</td><td>'+esc(r.path||'*')+'</td><td>'+esc(type)+'</td><td>'+target+'</td>'+
      '<td><button class="ghost" onclick="routeRemove('+i+')" title="remove">&times;</button></td></tr>';
  }).join('');
...
// ---- live server (deploy target): point Cards/Query/Schema at a real running
// host/picowal app_server (or compatible picoweb /wal/*-shaped API) instead of
// the localStorage simulator above. Off by default -- the simulator keeps
// working unchanged when no live server URL is configured (see
// webide-deploy-native-host: keep_local_sim=true). The native engine only
// understands numeric pack ids (0..4095), so a pack NAME (as typed into
// #packname / used by schemas/<pack>.schema.json) is deterministically hashed
// to a numeric id via FNV-1a -- same hash primitive already used elsewhere in
// this codebase (Map.Hash) -- reserving 0/1/2 (picoweb's users/registry/schema
// convention) so a name never collides with those.
var LIVE_SERVER_KEY='picoscript.liveserver.url';
function liveServerUrl(){ try{ return (localStorage.getItem(LIVE_SERVER_KEY)||'').trim(); }catch(e){ return ''; } }
function liveServerSet(url){
  try{ if(url) localStorage.setItem(LIVE_SERVER_KEY,url); else localStorage.removeItem(LIVE_SERVER_KEY); }catch(e){}
  liveServerRenderStatus();
}
function liveServerRenderStatus(){
  var url=liveServerUrl();
  ['liveServerStatus','liveServerStatus2'].forEach(function(id){
    var el=document.getElementById(id); if(!el) return;
    el.textContent=url?('live: '+url):'offline (localStorage simulator)';
    el.style.color=url?'var(--accent)':'var(--muted)';
  });
}
function liveServerPrompt(){
  var cur=liveServerUrl();
  if(typeof prompt!=='function'){ return; }
  var url=prompt('Live server base URL (e.g. http://localhost:8090) -- blank to use the offline simulator',cur);
  if(url===null) return;
  liveServerSet(url.trim());
}
function packNameToId(name){
  var h=0x811c9dc5>>>0;
  for(var i=0;i<name.length;i++){ h^=name.charCodeAt(i)&0xFF; h=Math.imul(h,0x01000193)>>>0; }
  return 3+(h%1000); // avoid 0=users,1=pack registry,2=schema store (picoweb/host-picowal convention)
}
function liveFetch(path,opts){ return fetch(liveServerUrl()+path,opts); }
...
function cardRender(){
  ...
  var pack=curPack(), fields=schemaForPack(pack);
  if(!rbacCanAct(pack,'read')){ document.getElementById('cardlist').innerHTML=rowsHtml([],true,fields); cardMsg('role "'+rbacCurrentRole()+'" cannot read pack "'+pack+'"',true); return; }
  var rowPred=rbacRowPredicate(pack);
  var url=liveServerUrl();
  if(!url){
    var entries=STORE.all(pack);
    if(rowPred) entries=entries.filter(function(e){return rowPred(e[1]);});
    document.getElementById('cardlist').innerHTML=rowsHtml(entries,true,fields);
    return;
  }
  var pid=packNameToId(pack);
  liveFetch('/list/'+pid).then(function(r){return r.json();}).then(function(d){
    var entries=(d.records||[]).map(function(rr){return [rr.record,rr.data];});
    if(rowPred) entries=entries.filter(function(e){return rowPred(e[1]);});
    document.getElementById('cardlist').innerHTML=rowsHtml(entries,true,fields);
  }).catch(function(e){ cardMsg('live list failed: '+e.message,true); });
}
function cardCreate(){
  var pack=curPack(),rec;
  try{rec=JSON.parse(document.getElementById('cardjson').value);}catch(e){cardMsg('JSON: '+e.message,true);return;}
  if(!rbacCanAct(pack,'create')){ cardMsg('role "'+rbacCurrentRole()+'" cannot create in pack "'+pack+'"',true); return; }
  var url=liveServerUrl();
  if(!url){
    try{var id=STORE.create(pack,rec);var hex=STORE.cardBytesHex(pack,id);document.getElementById('serout').textContent=hex;cardMsg('#'+id+' created',false);cardRender();}catch(e){cardMsg(e.message,true);}
    return;
  }
  var pid=packNameToId(pack);
  liveFetch('/'+pid,{method:'POST',body:JSON.stringify(rec)}).then(function(r){
    if(!r.ok) return r.json().then(function(d){throw new Error(d.error||('HTTP '+r.status));});
    return r.json();
  }).then(function(d){ document.getElementById('serout').textContent='(live)'; cardMsg('#'+d.record+' created (live)',false); cardRender(); })
    .catch(function(e){ cardMsg(e.message,true); });
}
...
function cardSeed(){
  var pack=curPack(), seed=[{qty:42,sku:"ABC",status:1},{qty:7,sku:"XYZ",status:0},{qty:99,sku:"ABC",status:1},{qty:55,sku:"QRS",status:2}];
  if(!rbacCanAct(pack,'create')){ cardMsg('role "'+rbacCurrentRole()+'" cannot create in pack "'+pack+'"',true); return; }
  var url=liveServerUrl();
  if(!url){ seed.forEach(function(r){STORE.create(pack,r);}); cardMsg('Seeded 4',false); cardRender(); return; }
  var pid=packNameToId(pack);
  Promise.all(seed.map(function(r){ return liveFetch('/'+pid,{method:'POST',body:JSON.stringify(r)}); }))
    .then(function(){ cardMsg('Seeded 4 (live)',false); cardRender(); })
    .catch(function(e){ cardMsg('live seed failed: '+e.message,true); });
}
function cardClear(){
  var pack=curPack();
  if(!rbacCanAct(pack,'delete')){ cardMsg('role "'+rbacCurrentRole()+'" cannot delete in pack "'+pack+'"',true); return; }
  var url=liveServerUrl();
  if(!url){
    STORE.all(pack).forEach(function(e){STORE.delete(pack,e[0]);});
    STORE.b.remove(pack+":ids");STORE.b.remove(pack+":next");
    document.getElementById('qresults').innerHTML='';cardMsg('Cleared',false);cardRender();
    return;
  }
  var pid=packNameToId(pack);
  liveFetch('/list/'+pid).then(function(r){return r.json();}).then(function(d){
    var ids=(d.records||[]).map(function(rr){return rr.record;});
    return Promise.all(ids.map(function(id){ return liveFetch('/'+pid+'/'+id,{method:'DELETE'}); }));
  }).then(function(){ document.getElementById('qresults').innerHTML=''; cardMsg('Cleared (live)',false); cardRender(); })
    .catch(function(e){ cardMsg('live clear failed: '+e.message,true); });
}
function cardDelete(id){
  var pack=curPack();
  if(!rbacCanAct(pack,'delete')){ cardMsg('role "'+rbacCurrentRole()+'" cannot delete in pack "'+pack+'"',true); return; }
  var url=liveServerUrl();
  if(!url){ STORE.delete(pack,id); cardRender(); return; }
  var pid=packNameToId(pack);
  liveFetch('/'+pid+'/'+id,{method:'DELETE'}).then(function(){ cardRender(); }).catch(function(e){ cardMsg('live delete failed: '+e.message,true); });
}
function cardQuery(){
  var pack=curPack(),q=document.getElementById('querybox').value,fields=schemaForPack(pack);
  queryRenderHead(); queryRenderChips();
  if(!rbacCanAct(pack,'read')){ document.getElementById('qresults').innerHTML=rowsHtml([],false,fields); cardMsg('role "'+rbacCurrentRole()+'" cannot read pack "'+pack+'"',true); return; }
  var rowPred=rbacRowPredicate(pack);
  var url=liveServerUrl();
  if(!url){
    try{
      var res=STORE.query(pack,q);
      if(rowPred) res=res.filter(function(e){return rowPred(e[1]);});
      document.getElementById('qresults').innerHTML=rowsHtml(res,false,fields);cardMsg(res.length+' match'+(res.length===1?'':'es'),false);
    }catch(e){cardMsg(e.message,true);}
    return;
  }
  // Live mode fetches the full list and applies the SAME query DSL client-side
  // (PicoStore.compileQuery) rather than reimplementing AND/OR/comparisons
  // server-side -- the native engine's own Storage.QueryCard only understands
  // a single field=value filter (see host-picowal-report-joins), so this keeps
  // full query-language parity between live and offline modes. Any RLS row
  // policy is ANDed on as an additional predicate, same as offline mode.
  var pid=packNameToId(pack);
  liveFetch('/list/'+pid).then(function(r){return r.json();}).then(function(d){
    var entries=(d.records||[]).map(function(rr){return [rr.record,rr.data];});
    var pred=PicoStore.compileQuery(q);
    var res=entries.filter(function(e){ return pred(e[1]) && (!rowPred||rowPred(e[1])); });
    document.getElementById('qresults').innerHTML=rowsHtml(res,false,fields);
    cardMsg(res.length+' match'+(res.length===1?'':'es')+' (live)',false);
  }).catch(function(e){ cardMsg(e.message,true); });
}
...
function sendRequest(){
  ...
  var routed=matchRoute(req.method,req.path);
  if(routed&&(routed.type==='query'||routed.type==='list')){ sendRequestDataRoute(routed,req); return; }
  var cl;
  try{cl=routed?fileCompilable(filesRead()[routed.file]):toCompilable();var r=PicoCompile.compile(cl.src,cl.lang);}catch(e){document.getElementById('respout').textContent='compile error: '+(e.message||e);return;}
  var vm=new PicoVM({cards:wal});vm.run(r.words);renderResponse(vm,req);
  if(routed){var el=document.getElementById('respout');el.textContent='\u2192 routed to '+routed.file+'\n'+el.textContent;}
  renderWal();busEmit('http.request',{method:req.method,path:req.path,status:vm.httpStatus,route:routed?routed.file:null});
}
// 'query'/'list' program exits don't run through the compile+VM path at all --
// they're a thin JSON wrapper directly over PicoStore (the same engine backing
// the Cards/Query tool), matching picoweb's own /wal/query and /wal/list
// conventions. Kept synchronous/offline-only here (the HTTP Simulator is a
// design-time tool; a deployed program exit against a live server is the
// native engine's own /query/{pack} and /list/{pack} routes, see
// host/picowal/app_router.eng).
function sendRequestDataRoute(routed,req){
  var pack=routed.pack, fields=schemaForPack(pack), el=document.getElementById('respout'), body, label;
  try{
    if(routed.type==='list'){
      var entries=STORE.all(pack);
      body=JSON.stringify({pack:pack,records:entries.map(function(e){return {record:e[0],data:e[1]};}),count:entries.length});
      label='List('+pack+')';
    } else {
      var res=STORE.query(pack,routed.query||'');
      body=JSON.stringify({pack:pack,count:res.length,rows:res.map(function(e){var rec={id:e[0]};for(var k in e[1])rec[k]=e[1][k];return rec;})});
      label='Query('+pack+', '+JSON.stringify(routed.query||'')+')';
    }
  }catch(e){ el.textContent='route error: '+e.message; el.style.color='#ff7b72'; return; }
  el.style.color='#7ee787';
  el.textContent='\u2192 routed to '+label+'\nHTTP/1.1 200 OK\nContent-Type: application/json\n\n'+body;
  renderWal();
  busEmit('http.request',{method:req.method,path:req.path,status:200,route:routed.type+':'+pack});
}
...
function selectToolTab(tab){
  ...
  if(TOOL_TAB==='routes')try{routesRender();}catch(e){}
}
...
function schemaSetModel(d){ setSrc(JSON.stringify(d,null,2)); schemaRenderDesigner(); if(typeof psFilesSave==='function')psFilesSave(); else if(typeof filesRender==='function')filesRender(); }
...
function schemaApplyJson(){
  ...
  if(typeof psFilesSave==='function') psFilesSave(); else if(typeof filesRender==='function') filesRender();
}
...
// PUT the active schema file to the configured live server, so Cards/Query's
// live-mode validation (Storage.SetSchemaForPack, see host/picowal/
// storage_file.c) has the same schema the WebIDE Schema Designer is showing.
// The native engine natively understands this {fields:[{name,type}]} shape
// directly (schema_format=extend-picoweb decision) -- no translation needed.
function schemaPushLive(){
  var url=liveServerUrl();
  if(!url){ if(typeof alert==='function') alert('No live server configured -- set one from the Cards panel first.'); return; }
  var m=/^schemas\/(.+)\.schema\.json$/.exec(ACTIVE_FILE||'');
  if(!m){ if(typeof alert==='function') alert('Save this as schemas/<pack>.schema.json first.'); return; }
  var pid=packNameToId(m[1]);
  liveFetch('/schema/'+pid,{method:'PUT',body:getSrc()}).then(function(r){
    if(!r.ok) throw new Error('HTTP '+r.status);
    if(typeof filesStatus==='function') filesStatus('Pushed schema for "'+m[1]+'" to live server (pack '+pid+')');
  }).catch(function(e){ if(typeof filesStatus==='function') filesStatus('Push failed: '+e.message,true); });
}
...
function eventSetModel(d){ setSrc(JSON.stringify(d,null,2)); eventRenderDesigner(); if(typeof psFilesSave==='function')psFilesSave(); else if(typeof filesRender==='function')filesRender(); }
...
function ontologySetModel(d){ setSrc(JSON.stringify(d,null,2)); ontologyRenderDesigner(); if(typeof psFilesSave==='function')psFilesSave(); else if(typeof filesRender==='function')filesRender(); }
...
// ---- RBAC / row-level security designer -------------------------------------
// A *.rbac.json file is {roles:[name,...], permissions:[{role,pack,actions:[...]}],
// rowPolicies:[{pack,role,predicate}]}. `actions` is a subset of
// ['read','create','update','delete']; `predicate` reuses the EXACT same query
// syntax as the Query panel/PicoStore.compileQuery (e.g. "status = 1"), so a
// row policy is just an extra filter ANDed onto whatever the user queries.
// Enforcement is client-side simulation for BOTH the offline localStorage
// simulator and live-server mode (see rbacCheckAction/rbacRowPredicate,
// wired into Cards create/update/delete and Query) -- this is a design-time
// preview of access control, not a production auth boundary; a real deployed
// server would need its own enforcement (see program-exits/live-picoweb-hosting).
var RBAC_ROLE_KEY='picoscript.rbac.role';
function rbacCurrentRole(){ try{ return localStorage.getItem(RBAC_ROLE_KEY)||''; }catch(e){ return ''; } }
function rbacSetCurrentRole(role){ try{ if(role) localStorage.setItem(RBAC_ROLE_KEY,role); else localStorage.removeItem(RBAC_ROLE_KEY); }catch(e){} }
// Find the *.rbac.json file governing a given pack (first file whose
// permissions/rowPolicies mention it; a repo typically has one rbac file).
function rbacModelForPack(pack){
  var files=(typeof filesRead==='function')?filesRead():{};
  for(var name in files){
    if(fileKind(name)!=='rbac') continue;
    try{
      var d=JSON.parse(files[name].src);
      var mentions=(d.permissions||[]).some(function(p){return p.pack===pack;})||(d.rowPolicies||[]).some(function(p){return p.pack===pack;});
      if(mentions) return d;
    }catch(e){}
  }
  return null;
}
// true if `role` may perform `action` (read|create|update|delete) on `pack`.
// No matching RBAC model or no matching permission row = unrestricted (RBAC
// is opt-in, matching Cards/Query's existing "schema-bound is opt-in" pattern).
function rbacCanAct(pack,action){
  var role=rbacCurrentRole(); if(!role) return true;
  var model=rbacModelForPack(pack); if(!model) return true;
  var perms=(model.permissions||[]).filter(function(p){return p.pack===pack&&p.role===role;});
  if(!perms.length) return true;
  return perms.some(function(p){return (p.actions||[]).indexOf(action)>=0;});
}
// Combined row-level predicate (AND of every matching rowPolicy) for the
// current role on `pack`, or null if none apply (no additional filtering).
function rbacRowPredicate(pack){
  var role=rbacCurrentRole(); if(!role) return null;
  var model=rbacModelForPack(pack); if(!model) return null;
  var policies=(model.rowPolicies||[]).filter(function(p){return p.pack===pack&&p.role===role&&p.predicate;});
  if(!policies.length) return null;
  var preds=policies.map(function(p){return PicoStore.compileQuery(p.predicate);});
  return function(rec){ return preds.every(function(pred){return pred(rec);}); };
}
function rbacGetModel(){
  try{ var d=JSON.parse(getSrc()); if(d&&typeof d==='object') return {roles:d.roles||[],permissions:d.permissions||[],rowPolicies:d.rowPolicies||[]}; }catch(e){}
  return {roles:[],permissions:[],rowPolicies:[]};
}
function rbacSetModel(d){ setSrc(JSON.stringify(d,null,2)); rbacRenderDesigner(); if(typeof psFilesSave==='function')psFilesSave(); else if(typeof filesRender==='function')filesRender(); }
var RBAC_ACTIONS=['read','create','update','delete'];
function rbacAddRole(){
  var el=document.getElementById('rbacRoleName'); var name=(el?el.value:'').trim();
  if(!/^[A-Za-z_][A-Za-z0-9_]*$/.test(name)){ if(typeof alert==='function') alert('role name must be a simple identifier'); return; }
  var d=rbacGetModel();
  if(d.roles.indexOf(name)<0) d.roles=d.roles.concat([name]);
  rbacSetModel(d);
  if(el) el.value='';
}
function rbacRemoveRole(name){
  var d=rbacGetModel();
  d.roles=d.roles.filter(function(r){return r!==name;});
  d.permissions=d.permissions.filter(function(p){return p.role!==name;});
  d.rowPolicies=d.rowPolicies.filter(function(p){return p.role!==name;});
  rbacSetModel(d);
}
function rbacAddPermission(){
  var roleEl=document.getElementById('rbacPermRole'), packEl=document.getElementById('rbacPermPack');
  var role=roleEl?roleEl.value:'', pack=(packEl?packEl.value:'').trim();
  if(!role||!pack){ if(typeof alert==='function') alert('pick a role and enter a pack name'); return; }
  var actions=RBAC_ACTIONS.filter(function(a){ var cb=document.getElementById('rbacAct_'+a); return cb&&cb.checked; });
  var d=rbacGetModel();
  d.permissions=d.permissions.filter(function(p){return !(p.role===role&&p.pack===pack);}).concat([{role:role,pack:pack,actions:actions}]);
  rbacSetModel(d);
  if(packEl) packEl.value='';
}
function rbacRemovePermission(idx){ var d=rbacGetModel(); d.permissions=d.permissions.filter(function(_,i){return i!==idx;}); rbacSetModel(d); }
function rbacAddRowPolicy(){
  var roleEl=document.getElementById('rbacPolRole'), packEl=document.getElementById('rbacPolPack'), predEl=document.getElementById('rbacPolPred');
  var role=roleEl?roleEl.value:'', pack=(packEl?packEl.value:'').trim(), predicate=(predEl?predEl.value:'').trim();
  if(!role||!pack||!predicate){ if(typeof alert==='function') alert('role, pack and predicate are all required'); return; }
  var d=rbacGetModel();
  d.rowPolicies=(d.rowPolicies||[]).concat([{pack:pack,role:role,predicate:predicate}]);
  rbacSetModel(d);
  if(predEl) predEl.value='';
}
function rbacRemoveRowPolicy(idx){ var d=rbacGetModel(); d.rowPolicies=d.rowPolicies.filter(function(_,i){return i!==idx;}); rbacSetModel(d); }
function rbacRenderDesigner(){
  var host=document.getElementById('rbacBody'); if(!host) return;
  var d=rbacGetModel(), roles=d.roles||[], perms=d.permissions||[], pols=d.rowPolicies||[];
  var roleOpts=roles.map(function(r){return '<option>'+esc(r)+'</option>';}).join('');
  var html='<div style="font-weight:600;margin-bottom:4px">Roles</div>';
  html+='<div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:8px">'+
    (roles.length?roles.map(function(r){return '<span class="wf-warn" style="display:inline-flex;align-items:center;gap:4px;padding:2px 8px;border-radius:10px;background:#2c313f">'+esc(r)+' <button class="ghost" style="padding:0 4px" onclick="rbacRemoveRole(\''+esc(r)+'\')">&times;</button></span>';}).join(''):'<span class="muted">no roles yet</span>')+
    '</div>';
  html+='<div style="display:flex;gap:6px;align-items:center;margin-bottom:14px">'+
    '<input id="rbacRoleName" placeholder="role name" style="width:130px" onkeydown="if(event.key===\'Enter\')rbacAddRole();">'+
    '<button class="ghost" onclick="rbacAddRole()">+ role</button></div>';

  html+='<div style="font-weight:600;margin-bottom:4px">Permissions (per role/pack action set)</div>';
  html+='<table class="wal" style="margin-bottom:8px"><thead><tr><th>role</th><th>pack</th><th>actions</th><th></th></tr></thead><tbody>';
  if(!perms.length) html+='<tr><td colspan="4" style="color:var(--muted)">no permissions yet (unrestricted by default)</td></tr>';
  perms.forEach(function(p,i){
    html+='<tr><td>'+esc(p.role)+'</td><td>'+esc(p.pack)+'</td><td>'+esc((p.actions||[]).join(', ')||'(none)')+'</td>'+
      '<td><button class="ghost" onclick="rbacRemovePermission('+i+')" title="remove">&times;</button></td></tr>';
  });
  html+='</tbody></table><div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap;margin-bottom:14px">'+
    '<select id="rbacPermRole"><option value="">role&hellip;</option>'+roleOpts+'</select>'+
    '<input id="rbacPermPack" placeholder="pack name" style="width:120px">'+
    RBAC_ACTIONS.map(function(a){return '<label style="font-size:11px"><input type="checkbox" id="rbacAct_'+a+'"> '+a+'</label>';}).join(' ')+
    '<button class="ghost" onclick="rbacAddPermission()">+ permission</button></div>';

  html+='<div style="font-weight:600;margin-bottom:4px">Row-level security policies</div>';
  html+='<table class="wal" style="margin-bottom:8px"><thead><tr><th>pack</th><th>role</th><th>predicate</th><th></th></tr></thead><tbody>';
  if(!pols.length) html+='<tr><td colspan="4" style="color:var(--muted)">no row policies yet (full row access by default)</td></tr>';
  pols.forEach(function(p,i){
    html+='<tr><td>'+esc(p.pack)+'</td><td>'+esc(p.role)+'</td><td><code>'+esc(p.predicate)+'</code></td>'+
      '<td><button class="ghost" onclick="rbacRemoveRowPolicy('+i+')" title="remove">&times;</button></td></tr>';
  });
  html+='</tbody></table><div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap">'+
    '<input id="rbacPolPack" placeholder="pack name" style="width:110px">'+
    '<select id="rbacPolRole"><option value="">role&hellip;</option>'+roleOpts+'</select>'+
    '<input id="rbacPolPred" placeholder="status = 1 (Query syntax)" style="width:200px" onkeydown="if(event.key===\'Enter\')rbacAddRowPolicy();">'+
    '<button class="ghost" onclick="rbacAddRowPolicy()">+ policy</button></div>';
  host.innerHTML=html;
}
function rbacToggle(active,name){
  var host=document.getElementById('rbacDesigner');
  if(host) host.style.display=active?'block':'none';
  if(!active){ rbacCloseJson(); return; }
  var mon=document.getElementById('monaco'), src=document.getElementById('src');
  if(mon) mon.style.display='none'; if(src) src.style.display='none';
  try{ rbacRenderDesigner(); }catch(e){}
}
function rbacOpenJson(){
  var m=document.getElementById('rbacJsonModal'), t=document.getElementById('rbacJsonText'), e=document.getElementById('rbacJsonErr');
  if(!m||!t) return; if(e) e.textContent='';
  t.value=getSrc(); m.classList.add('open'); t.focus();
}
function rbacCloseJson(){ var m=document.getElementById('rbacJsonModal'); if(m) m.classList.remove('open'); }
function rbacApplyJson(){
  var t=document.getElementById('rbacJsonText'), e=document.getElementById('rbacJsonErr');
  if(!t) return;
  try{ var d=JSON.parse(t.value); if(!d||typeof d!=='object') throw new Error('expected {roles:[...],permissions:[...],rowPolicies:[...]}'); }
  catch(err){ if(e){ e.textContent='invalid JSON: '+String(err.message||err); e.style.color='#ff7b72'; } return; }
  setSrc(t.value); rbacCloseJson();
  try{ rbacRenderDesigner(); }catch(_){}
  if(typeof psFilesSave==='function') psFilesSave(); else if(typeof filesRender==='function') filesRender();
}
// Create a new *.rbac.json file and open it straight into the designer.
function rbacQuickNew(){
  var files=filesRead(), base='access', name=base+'.rbac.json', n=2;
  while(files[name]){ name=base+'-'+(n++)+'.rbac.json'; }
  files[name]={kind:'rbac',lang:'',src:JSON.stringify({roles:[],permissions:[],rowPolicies:[]},null,2),updated:Date.now()};
  filesWrite(files); filesRender();
  psFilesOpen(name);
  return name;
}
...
initMonaco();compileSrc(false);loadSample();renderWal();renderLayout();busRender();liveServerRenderStatus();
(function(){var el=document.getElementById('rbacRoleInput'); if(el) el.value=rbacCurrentRole();})();
