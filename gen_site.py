...
function cardRender(){
  ...
  var url=liveServerUrl();
  if(!url){ document.getElementById('cardlist').innerHTML=rowsHtml(STORE.all(curPack()),true,fields); return; }
  var pid=packNameToId(curPack());
...
function cardQuery(){
  ...
  // full query-language parity between live and offline modes.
  var pid=packNameToId(pack);
  liveFetch('/list/'+pid).then(function(r){return r.json();}).then(function(d){
    var entries=(d.records||[]).map(function(rr){return [rr.record,rr.data];});
    var pred=PicoStore.compileQuery(q);
    var res=entries.filter(function(e){ return pred(e[1]); });
